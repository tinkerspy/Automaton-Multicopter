...
void MPU6050jb::init( int16_t sample_interval_us ) {
  ...
  for (int cal_int = 0; cal_int < 2000 ; cal_int ++){                  //Run this code 2000 times
    requestSample();
    while ( !sampleReady() );
    readSample();                                              //Read the raw acc and gyro data from the MPU-6050
...
bool MPU6050jb::sampleAvailable( void ) { 
  return true; 
}

void MPU6050jb::requestSample( void ) {
  Wire.beginTransmission(0x68);                                        //Start communicating with the MPU-6050
...
bool MPU6050jb::sampleReady( void ) {
...
void MPU6050jb::computeAngles( void ) {
...
float MPU6050jb::angleX( void ) {
  return angle_pitch_output;
}

float MPU6050jb::angleY( void ) {
  return angle_roll_output;
}

float MPU6050jb::angleZ( void ) {
  return 0; // TODO
}
